...
impute_income <- function(data, dump_name = "inc_data") {
    ...
    wide_form <- data %>% select(-c(people_count, median)) %>% spread(Age, mean_income)
    print(colnames(wide_form))
    colnames(wide_form)[3:13] <- c('age_15_19', 'age_20_24', 'age_25_29', 'age_30_34', 'age_35_39', 'age_40_44', 'age_45_49', 'age_50_54', 'age_55_59', 'age_60_64', 'age_65_')
    print(colnames(wide_form))
    head(wide_form)
    ...
    level_15_19_mean_incomes <- data %>% filter(Age == "15-19 Years" & !is.na(mean_income)) %>% 
        ...
        mutate(level_code = substring(Level, 1, 2)) %>% 
        filter(as.numeric(level_code) < 7)
    level_15_19_mean_incomes
    ...
    higher_level_imputes <- tibble(level_code = c('07', '08', '09', '10'))
    ...
    ready_to_impute <- should_impute %>% mutate(level_code = substring(Level, 1, 2)) %>% 
