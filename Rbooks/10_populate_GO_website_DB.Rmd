...
```{r}
field_of_study_people_counts <- dbReadTable(go_analysis_con, DBI::Id(schema = "lng", table = "narrow_field_of_study_people_counts"))
head(field_of_study_people_counts)
```

```{r}
field_of_study_people_counts %>% filter(is.na(field_code))
```

```{r}
dbWriteTable(go_web_con, name = "NarrowFieldOfStudy",  
                field_of_study_people_counts %>%
                 filter(!is.na(TotalPeopleCount), !is.na(field_code)) %>%
                 mutate(NarrowFieldOfStudyCode = strtoi(field_code, base = 10L), BroadFieldOfStudyCode = strtoi(substring(field_code, 1,2), base = 10L)) %>%
                 rename(ClassificationTitle = field) %>%
...
```{r}
lifetime_earnings_data <- dbReadTable(go_analysis_con, DBI::Id(schema = "lng", table = "cen_lifetime_income2")) %>%
    mutate(NarrowFieldOfStudyCode = substring(Field_Text, 1, 4), NZQALevel = strtoi(level_code, base = 10L), EarnedAboveSchoolLeaver = as.integer(lifetime_above_school_leaver)) %>%
...
```{r}
dbWriteTable(go_web_con, name = "NarrowFieldOfStudyLevel", narrow_field_of_study_level %>% filter(!is.na(NZQALevel)), append = TRUE)
```
...
```{r}
annual_income_table <- fulltime_30_39_income %>% rbind(fulltime_30_39_income_all) %>% 
    mutate(
        NZQALevel = strtoi(level_code, base = 10L),
        PeopleCount = as.integer(people_count),
        Income = as.integer(mean_income),
...
```{r}
TestDb <- DBI::dbConnect(odbc::odbc(),
                      Driver = "SQL Server", 
                      Server = "localhost\\SQLEXPRESS",
                      Database = "TestDb",
                      Trusted_Connection = "True")
...
dbWriteTable(
    go_web_con, 
    name = "ANZSCOOccupationL5NarrowFieldOfStudyAlternative", 
    dbReadTable(TestDb, name="ANZSCOOccupationL5NarrowFieldOfStudyAlternative") %>% filter(Level5ANZSCOCode > 3, !Level5ANZSCOCode %in% retired_codes$Level5ANZSCOCode) %>% select(-c(Id)),
    append = TRUE
)
```
