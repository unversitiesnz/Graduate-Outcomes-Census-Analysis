...
```{r}
occupation_studylevel <- read.csv("../input_data/occupations_by_levels_broad_2018.csv")
occupation_studylevel$people_count <- as.numeric(occupation_studylevel$people_count)
head(occupation_studylevel, 10)
```


```{r}
occupation_studylevel <- occupation_studylevel %>% 
    append_study_level_code(level_var = "i_standard_highest_qual_code", code_var = "level_code") %>% select(-i_standard_highest_qual_code) %>% rename(i_standard_highest_qual_code=level_code)
...
occupation_counts <- occupation_studylevel %>% filter(
    !i_standard_highest_qual_code %in% c("Overseas Secondary School Qualification", "Not elsewhere included")
...
result <- occupation_studylevel_wide %>%
        group_by(i_occupation_code) %>%
        mutate(school = sum(lvl_00, lvl_01, lvl_02, lvl_03, lvl_11, na.rm = TRUE), 
               cert_dip = sum(lvl_05, na.rm = TRUE),
               degree_or_higher = sum(lvl_07, lvl_08, lvl_09, lvl_10, na.rm = TRUE), 
               tertiary = sum(lvl_04, lvl_05, lvl_07, lvl_08, lvl_09, lvl_10, na.rm = TRUE)
              ) %>% ungroup()
head(result)
```
