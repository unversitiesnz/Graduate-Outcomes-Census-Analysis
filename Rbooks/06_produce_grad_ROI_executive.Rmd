...
The main goal of this notebook is to create an excel workbook containing a couple of sheets used by the excutive director (Chris) to answer ad-hoc queries.
...
#Adjusting format of columns, bringing in text variables for occupating and narrow field of study
```{r}
...
```{r}
income_table <- consolidated_income %>% mutate(narrow_field_code = substr(Field_Text,1,4)) %>%
    rename(
...
top_occupations_3 <- income_table %>% right_join(top_occupations_2, by=c("narrow_field_code", "i_standard_highest_qual_code")) %>% 
    ...
    select(-Field_Text) %>% select(field_id, narrow_field_code, i_standard_highest_qual_code, everything())
...
addStyle(wb, occupation_sheet_name, openxlsx::createStyle(textDecoration = "bold", wrapText = TRUE), rows = rowRange, cols = 1, gridExpand = TRUE)
```
