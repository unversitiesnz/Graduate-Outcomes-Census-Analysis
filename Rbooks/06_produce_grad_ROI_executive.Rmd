...
occupation_cross_ref_table_1 <- grad_cube2 %>% mutate(
    ...
    arrange(field_id) %>%
    full_join(top_occupations_3 %>% rename(`Narrow Field of Study` = field_id), by=c("field_id" = "Narrow Field of Study"))
head(occupation_cross_ref_table_1)
```

```{r}
occupation_table_5 <- occupation_cross_ref_table_1 %>% select(-i_standard_highest_qual_code) %>% t %>% as.data.frame(stringsAsFactors = FALSE) 
