...
The main goal of this notebook is to create a formatted excel workbook containing a couple of sheets used by the executive director (Chris) to answer ad-hoc queries.
...
consolidated_income <- dbReadTable(go_analysis_con, Id(schema = "lng", table = "consolidated_income")) %>%
    mutate(age_when_better_off = ifelse(ever_better_off, age_when_better_off, "NEVER")) %>% filter(employed_code == "1") %>% select(-employed_code)
