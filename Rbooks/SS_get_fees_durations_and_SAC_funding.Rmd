---
title: "Screen Scrap Fees, Durations and SAC funding"
...
When you run the first python chuck R will ask to install Miniconda. I would recommend that you do this.
Once Minicoda is installed open (from the start menu) 'Anaconda Powershell Prompt (R-Mini)' run the following command
...
This process is more robust than the Careers NZ screen scrapping becuase all the required information is on a single page, in one table.
...
You can go to: https://chromedriver.chromium.org/downloads to download the webdriver. You will also need to have a somewhat upto date version of Chrome installed. 
...
# cells that where na are now NaN
```
...
def use_funding_code(x):
    ...
    if len(x) == 2:
        ...
        return x
use_funding_code("A1"), use_funding_code("A3/4"), use_funding_code("A1/B2")
```
...
# ********************************************************
# CAREER NZ SCREEN SCRAPPING of fees and durations
# ********************************************************
...
## define functions to do the screen scrapping
```{python}
...
# Running the webscrapping
Webscrapping can take time, during the process a screen of chrome will run itself and navigate through pages on the Careers NZ website.
...
## Web scrapping all links to courses on the Careers NZ site
...
## Web scrapping details from all the courses pages on the Careers NZ website
...
## do some data cleaning of the course dataset
```{python}
...
## Remove enteries which have to useful data
```{python}
...
# ***********************************************
# R section: finalise and do some fulling of the gaps in the fees and durations, then merge with SAC funding rates
# ***********************************************
...
## Use sensable values to full gaps
```{r}
fees_full <- tibble(
...
fees2 <- fees_and_durations2 %>% 
    mutate(
        ...
        none_fee_component = 10900 * duration_yr
    ) %>% mutate(
...
```{r}
write.csv(sac_paid, "../datasets/fees_durations_SAC.csv", row.names = FALSE)
```
