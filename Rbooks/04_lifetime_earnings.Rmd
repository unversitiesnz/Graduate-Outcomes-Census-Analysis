...
Simple in that it does not remove fees from the total income. Assume people 
...
```{r}
filter(simplistic_lifetime_earnings, employed_code==1, level_code == '00')
```
...
```{r}
fees_and_funding <- read.csv("../program_costs/fees_durations_SAC_NEW.csv") %>% mutate(
...
tax_calculated <- income_with_fees %>% group_by(employed_code, field, level) %>% arrange(age) %>% 
    mutate( #Offset income = income, unless younger than minimum qual completion age
        offset_income = ifelse(is.na(duration_whole_yr), gross_income, ifelse(age < 17 + duration_whole_yr, 0, gross_income)),
         #Loan payments are 12% of all gross income over $19760/year
...
Calculating lifetime earnings for school leavers, for reference
```{r}
baseline_income %>% group_by(employed_code) %>% summarise(lifetime_earnings = sum(baseline_income))
```
...
# Lifetime totals of income above, tax above, etc.
```{r}
...
dbWriteTable(go_analysis_con, Id(schema = "lng", table = "annual_npv_18"), npv_calculated, 
             overwrite = TRUE)
```
...
```{r}
# do some checks
npv_calculated %>% filter(field_code == "0000", employed_code == 1) %>% View()
```
