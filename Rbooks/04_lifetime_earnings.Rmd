...
output_field_levels_all <- function(.data, con, value_var, table_name) {
    tabulated_income_b <- .data %>% pivot_wider(id_cols = c("level_code", "Field_Text"), names_from = level_code,names_prefix = "lvl ", values_from = value_var)
...
```{r}
employed_code = '2'
yearly_gross_income_long <- dbGetQuery(go_analysis_con, paste0("SELECT *
  FROM [lng].[cen_yearly_gross_income]
  where employed_code = ", employed_code))
...
```{r}
total_people_counts <- dbGetQuery(go_analysis_con, paste0("SELECT
      [level_code]
      ,[field], field_code
,[people_count2]
  FROM [GO_Analysis_Data].[lng].[cen_annual_avg_income]
  where age = 'Total' and people_count > 0 and set_standard = 1 and employed_code = ", employed_code)) %>% rename(people_count_all_ages = people_count2) %>% mutate(Field_Text = paste(field_code, field)) %>%
...
simplistic_lifetime_earnings %>% 
    output_field_levels_all(con = go_analysis_con, value_var = "simple_lifetime_income", table_name = paste0("cen_lifetime_earnings_a", employed_code))
```
...
```{r}
tax_calculated <- income_with_fees %>% group_by(field, level) %>% arrange(age) %>% 
    mutate( #Offset income = income, unless younger than minimum qual completion age
        ...
        loan_repayments = ifelse( # If the total loan contributions minus current one are greater than loan, repayment = 0. Otherwise, if 
            cumsum(offset_income * 0.12) - (offset_income * 0.12) >= loan_at_graduation, 
            0,
            ifelse(cumsum(offset_income * 0.12) >= loan_at_graduation,
                   loan_at_graduation - (cumsum(offset_income * 0.12) - (offset_income * 0.12)),
                   (offset_income * 0.12)
                   )
        )
...
```{r}
components_calculated <- tax_calculated %>% left_join (baseline_income, by="age") %>%
    mutate(
        above_school_leaver = offset_income - baseline_income,
...
summarised_lifetime_income <- npv_calculated %>% mutate(Field_Text = paste(field_code, field)) %>% 
    group_by(Field_Text, level_code) %>%
    summarise(
...
```{r}
dbWriteTable(go_analysis_con, Id(schema = "lng", table = paste0("cen_lifetime_income", employed_code)), 
...
```{r}
tabulated_income_b <- summarised_lifetime_income %>% pivot_wider(id_cols = c("level_code", "Field_Text"), names_from = level_code,names_prefix = "lvl ", values_from = "lifetime_above_school_leaver")
print(tabulated_income_b)
dbWriteTable(go_analysis_con, Id(schema = "tab", table = paste0("cen_lifetime_earnings_above_school_leaver", employed_code)), tabulated_income_b, overwrite = TRUE)
```

```{r}
summarised_lifetime_income %>%
    output_field_levels_l4_l10(con = go_analysis_con, value_var = "lifetime_above_school_leaver", paste0("cen_lifetime_earnings_above_school_leaver", employed_code)) %>%
    output_field_levels_l4_l10(con = go_analysis_con, value_var = "discounted_lifetime_income", paste0("cen_lifetime_discounted_earnings", employed_code)) %>%
    output_field_levels_l4_l10(con = go_analysis_con, value_var = "discounted_lifetime_above_school_leaver", paste0("cen_lifetime_discounted_earnings_above_school_leaver", employed_code)) %>%
    output_field_levels_l4_l10(con = go_analysis_con, value_var = "offset_lifetime_income", paste0("cen_lifetime_offset", employed_code)) %>%
    output_field_levels_l4_l10(con = go_analysis_con, value_var = "lifetime_tax", paste0("cen_lifetime_tax", employed_code)) %>%
    output_field_levels_l4_l10(con = go_analysis_con, value_var = "tax_above_school_leaver", paste0("cen_tax_above_school_leaver", employed_code)) %>%
    output_field_levels_l4_l10(con = go_analysis_con, value_var = "npv_tax_over_school_leaver", paste0("cen_npv_tax_over_school_leaver", employed_code)) %>%
    head()
```
...
mutate(
    ...
    select(Field_Text, level_code, age, better_off) %>%
...
tabulated_age_better_off <- age_when_better_off %>% 
    pivot_wider(id_cols = c("level_code", "Field_Text"), names_from = level_code,names_prefix = "lvl ", values_from = "age") %>%
    select(Field_Text, `lvl 04`, `lvl 05`, `lvl 07`, `lvl 08`, `lvl 09`, `lvl 10`)

print(tabulated_age_better_off)
dbWriteTable(go_analysis_con, Id(schema = "tab", table = paste0("cen_age_better_off", employed_code)), tabulated_age_better_off, overwrite = TRUE)
```
...
```{r}
dbWriteTable(go_analysis_con, Id(schema = "lng", table = "age_better_off"), age_when_better_off)
```