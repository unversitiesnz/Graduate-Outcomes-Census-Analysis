...
get_30_39_income_summary_set <- function (.data) {
    temp <- .data %>% 
      ...
      #Calculating unweighted mean - weighted means calculated later
        summarise(mean_income = round2(mean(mean_income2, na.rm = TRUE), -2), people_count = sum(people_count2, na.rm = TRUE)) %>%
        mutate(
...
```{r}
income_data <- data %>%
#    rename(mean_income = mean_income2, people_count = people_count2) %>%
  filter(people_count2 > 3) %>% #Filtering here and in following line to remove very low, unpredictable observations
