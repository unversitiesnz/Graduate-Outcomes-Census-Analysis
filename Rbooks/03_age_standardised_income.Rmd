...
get_income_age_distribution <- function(.data) {
    age_distribution <- .data %>% group_by(age) %>% 
      summarise(people_count = sum(people_count, na.rm = TRUE)) %>%
      mutate(proportion = people_count/sum(.data$people_count, na.rm = TRUE))
...
## Convert income to long
```{r}
income_long %>% mutate(
    age = convert_to_unsafe_age[age]
) %>% left_join(age_distribution, by="age") %>%
group_by(level_code, level, field_code, field)
```
...
**TODO: Output long table to database** 

# Output data
```{r}
tabulated_income_a <- age_standard_income %>% pivot_wider(id_cols = c("level_code", "Field_Text"), names_from = level_code,names_prefix = "lvl ", values_from = "age_sd_income")
...
```{r}
tabulated_counts <- age_standard_income %>% pivot_wider(id_cols = c("level_code", "Field_Text"), names_from = level_code,names_prefix = "lvl ", values_from = "people_count")
